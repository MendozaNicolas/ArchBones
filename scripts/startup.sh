...
set_password() {
    read -rs -p "Ingresá una contraseña: " PASSWORD1
    echo -ne "\n"
    read -rs -p "Reingresá la contraseña: " PASSWORD2
    echo -ne "\n"
    if [[ "$PASSWORD1" == "$PASSWORD2" ]]; then
        set_option "$1" "$PASSWORD1"
    else
        echo -ne "ERROR! Las contraseñas no coinciden. \n"
        set_password
    fi
}
...
# @description Displays ArchBones logo
# @noargs
logo() {
...
"
    ...
    3) exit ;;
    *)
        echo "Opción incorrecta, intentá de nuevo"
        filesystem
        ;;
    esac
}
